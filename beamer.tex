...
\item Block can only include transactions whose total sum of gas is less than 8 million \pause
\end{itemize}
}
...
\item Ethash PoW is memory hard, making it ASIC resistant \pause
...
\subsection{Fabric - System Architecture}
\frame{\frametitle{Fabric - System Architecture}
Includes :  \pause
\begin{itemize} 
\item Nodes \pause
\item Transactions \pause
\item Blockchain data structures \pause
\end{itemize}
...
\item peer receives ordered state updates in the form of blocks from the ordering service and maintain the state and the ledger.
}
